...
# Default model parameters (session-tuned only)
...
st.markdown(
    ...
    <p>
      Manual-line, usage & context-aware model for 2-pick props:
      <ul>
        ...
        <li>Personal bet history, CLV, and calibration-driven tuning</li>
      </ul>
    </p>
    """,
    unsafe_allow_html=True,
...
user_input = st.sidebar.text_input(
    ...
    key="user_id_input",  # unique key to avoid duplicate ID error
...
def get_team_context():
    ...
    try:
        ...
        pace_stats = LeagueDashTeamStats(
            season=current_season(),
            per_mode_detailed="PerGame"
        ).get_data_frames()[0][["TEAM_ABBREVIATION", "PACE"]]
...
def get_context_multipliers(team_abbrev: str | None, opp_abbrev: str | None):
    ...
    def_factor = 0.5 + 0.5 * def_factor  # soften extremes
...
def compute_profit(row):
    ...
    if "Combo" in bet_type:
        ...
        b = 1.0  # assume ~even odds for singles (for ROI tracking)
...
def compute_clv(row):
    """CLV = ClosingLine - EntryLine (for overs)."""
    try:
    ...
    if trigger:
        if payout_mult <= 1.0:
            ...
            if use_last:
                ...
                p1_opp_i = p1_opp.strip().upper()
                p2_opp_i = p2_opp.strip().upper()
            ...
            if p1_mu_min is None:
                ...
                if p2_mu_min is None:
                    ...
                    with col_a:
                        ...
                        st.markdown(f"**Suggested Stake:** ${p1_stake:.2f}")
                    ...
                    with col_b:
                        ...
                        st.markdown(f"**Suggested Stake:** ${p2_stake:.2f}")
                    ...
                    st.markdown(
                        f"**Correlation Applied**: {corr_reason}",
                        unsafe_allow_html=True,
                    )
                    st.markdown(f"**Joint Prob:** {joint_prob * 100:.1f}%")
                    st.markdown(f"**EV per $ (display):** {combo_ev_disp * 100:.1f}%")
    ...
    if df is None:
        ...
        editable = st.data_editor(
            ...
            disabled=[
                ...
                if c not in ["Result", "ClosingLine"]
            ],
            key="results_editor",
        )
    ...
    if df is None:
        ...
        with colf3:
            market_filter = st.selectbox(
                ...
                index=0,
                key="hist_market_filter",
            )

        name_search = st.text_input(
            "Filter by Player (optional)",
            value="",
            key="hist_name_search",
        ).strip().lower()
        ...
        # Market filter
        if market_filter != "All" and "Market" in hist.columns:
        ...
        # Name search
        if name_search and "Player" in hist.columns:
        ...
        if st.button("Export Filtered to CSV", use_container_width=True, key="export_filtered"):
...
with tab_calib:
    ...
    if df is None:
        ...
        calib["EV"] = pd.to_numeric(calib.get("EV", np.nan), errors="coerce")
        ...
        if len(calib) < 20:
            ...
            calib_df = pd.DataFrame({"Pred": bin_pred, "Actual": bin_actual}).dropna()
            ...
            avg_ev = calib["EV"].mean() if not calib["EV"].isna().all() else np.nan
            ...
            st.markdown(f"- **ROI (logged stakes):** {roi * 100:.1f}%")

            # CLV
            clv_vals = pd.to_numeric(calib.get("CLV", np.nan), errors="coerce").dropna()
            if not clv_vals.empty:
                st.markdown(f"- **Avg CLV:** {clv_vals.mean():+.2f}")
                st.markdown(
            ...
            # Underconfident
                ...
                recs.append("Model is underconfident → decrease variance slightly.")
                new_ht_pra = max(1.15, new_ht_pra - 0.05)
                new_ht_other = max(1.10, new_ht_other - 0.05)
            else:
                recs.append("Calibration gap is small → variance is reasonable.")

            # ROI vs EV
            if not np.isnan(avg_ev):
                if avg_ev > 0 and roi < 0:
                    recs.append(
                        "Positive EV but negative ROI → edges overstated. "
                        "Slightly tighten usage clamp (reduce max)."
                    )
                    ...
                    recs.append(
                        ...
                        "You may slightly relax variance or EV gate."
                    )
            ...
            st.markdown(
                ...
                <ul>
                  ...
                  <li>Usage clamp: <b>{st.session_state['usage_min']:.2f} - {st.session_state['usage_max']:.2f}</b>
                      → <b>{new_usage_min:.2f} - {new_usage_max:.2f}</b></li>
                </ul>
                <p style="font-size:0.85rem;color:#aaa;">
                Changes apply only this session and keep projections anchored to live stats.
                </p>
                </div>
                """,
                unsafe_allow_html=True,
            ...
            with col_apply:
                if st.button("✅ Apply Suggested Changes (Session Only)", use_container_width=True, key="apply_tuning"):
                    ...
                    st.success("Updated model parameters for this session. Re-run in Model tab.")
            with col_reset:
                if st.button("♻️ Reset to Default Parameters", use_container_width=True, key="reset_tuning"):
                    st.session_state["ht_pra"] = 1.35
                    st.session_state["ht_other"] = 1.25
                    st.session_state["usage_min"] = 0.7
                    st.session_state["usage_max"] = 1.4
                    st.success("Reset to baseline parameters for this session.")
...
st.caption(
    "Nothing is 100%. This engine gives you disciplined edges, context, and feedback. "
    "Your edge comes from selectivity + bankroll discipline, not forcing action."
)
