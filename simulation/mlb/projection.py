...
Given a player name, market, line, and (optional) game context, this:
  1. resolves the player and whether they're a batter or pitcher,
  2. builds their real profile (shrunk season rates),
  3. resolves the opponent (probable SP for a batter; opposing lineup for a
     pitcher), park, and weather,
  4. runs the at-bat Monte Carlo, and
  5. returns proj / p_over / p_under / distribution percentiles for the market.

Output schema is engine-agnostic so the app can price (EV, Kelly, gate) it the
same way it prices NBA legs.
"""
...
def _resolve_context(team_id, team_name, game_date):
    """Find today's game for the player's team: opponent, park, probable SPs."""
    import statsapi
    if not team_id:
        return {}
    try:
    ...
    # schedule() returns probable-pitcher NAMES only, not ids — resolve ids.
...
def project(name: str, market: str, line: float,
    ...
    if is_pitcher:
        ...
        # The pitcher's own team's lineup affects nothing for his K/outs/ER, but
        # the engine needs both sides. Pitcher is the AWAY/HOME starter facing opp.
        eng = MLBGameEngine(
        ...
        # Build the batter's own team lineup so Runs/RBI have real context.
        ...
        if opp_sp_id:
            opp_sp = dl.get_pitcher_profile(opp_sp_id, opp_sp_nm, season=game_date.year, is_starter=True)
        if not opp_sp:
    ...
    # PrizePicks has no push — redistribute push mass proportionally.
...
def _ensure_in_lineup(lineup: list[BatterProfile], batter: BatterProfile) -> list[BatterProfile]:
    ...
    # Replace the matching player (by id) with the freshly-built profile, or
    # insert into their natural slot.
    replaced = False
